...
#                    raise Exception( "Error: item %s is in category %s, will not add to category %s"%(item, item2category[item],cur_cat))
    ...
    for l in list(f) + ["EOF"]:
        try:
            ...
            if len(l)==0:
                continue
            if l.startswith(_autogen_str):
                gen_cmd=l[len(_autogen_str):]
            elif l.startswith(_hash_str):
                file_hash=l[len(_hash_str):]
                if (char.char_hash!=file_hash):
                    print("Warning: The file %s was created for character hash"
                          " %s, but the current character hash is %s.\n"
                          "  Rebuild with %s"
                          %(fname, file_hash, char.char_hash, gen_cmd))
            if l[0]=='#':
                continue
            if l[0]=='+':
                cur_cat=l[1:].strip()
